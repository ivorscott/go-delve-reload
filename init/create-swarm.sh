...
LEADER_IP=$(docker-machine ssh gdr1 ifconfig eth1 | grep 'inet addr' | cut -d: -f2 | awk '{print $1}')

# create a swarm as all managers
docker-machine ssh gdr1 docker swarm init --advertise-addr "$LEADER_IP"
...
JOIN_TOKEN=$(docker-machine ssh gdr1 docker swarm join-token -q manager)

# for i in 2 3; do
#   docker-machine ssh node$i docker swarm join --token "$JOIN_TOKEN" "$LEADER_IP":2377
# done

docker-machine env gdr1
